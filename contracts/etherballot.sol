...
contract EtherBallot {
    ...
    constructor(string[] memory _candidateNames, uint _startTime, uint _stopTime) {
    ...
    function vote(string calldata _voterName, string calldata _candidate) external payable {
        require(block.timestamp >= startTime && block.timestamp <= stopTime, "Voting is not allowed at this time.");
